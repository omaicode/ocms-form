 ...
 * Class Field.
 */
class BaseField implements Renderable
{
    use Macroable;
    ...
    {
        return $this->placeholder ?: trans('form::messages.input_placeholder').' '.$this->label;
    }
     ...
     * @return self
     */
    public function setFieldClass(array $fieldClass, $replace = false): self
    {
        $this->fieldClass = $replace ? $fieldClass : array_merge($this->fieldClass, $fieldClass);
    ...
    public function getView(): string
    {
        ...
        return 'form::fields.'.strtolower(end($class));
    }
    ...
    public function render()
    {
        ...
        if ($this->callback instanceof Closure && $this->form) {
        ...
        foreach($assets['css'] as $css) {
            if(!AdminAsset::has('css', $css)) {
                AdminAsset::addStyle(uniqid(), $css);
            }
        }
        foreach($assets['js'] as $js) {
            if(!AdminAsset::has('js', $js)) {
                AdminAsset::addScript(uniqid(), $js);
            }
        }
